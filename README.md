...
为 AI Agent 提供 MAPTEC JSAPI 服务能力的 Skill 集合，支持地图加载、交互、控件、事件、基础与高级可视化覆盖物、地点检索、路线规划和自定义图层等功能。
...
| 样式切换 | 白天、黑夜和 TMC 交通底图样式切换 | “切换到交通态势暗色地图” |
| 底图标注 | 底图文字/图标语言切换和显隐 | “底图显示中文并隐藏 POI 名称” |
| 事件监听 | 地图点击、缩放变化、中心点变化、覆盖物点击等 | “点击地图获取当前位置的经纬度” |
| Marker | Marker 覆盖物 | “在点击位置增加 Marker 点” |
| 矢量覆盖物 | 点、线、圆、多边形、GeoJSON，以及 Circle 特效与聚合 | “绘制雷达扫描点并聚合密集圆点” |
| 批量图标 | IconOverlay 图标、文字、碰撞避让和聚合 | “批量展示站点图标和名称” |
| 高级可视化 | 三维棱柱、二维/三维热力、三维蜂窝、区域遮罩 | “展示三维订单热力图” |
| 覆盖物管理 | 统一显隐、查询、批量添加/移除与清空 | “隐藏全部业务覆盖物” |
| 渲染要素查询 | 点击、框选、全视口查询可见要素 | “框选当前区域内的 POI” |
| 自定义图层 | 自定义 WebGL/Three.js 图层和三维模型 | “加载 GLTF 三维模型” |
